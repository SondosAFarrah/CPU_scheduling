...
void FCFS(struct SchedulerQueue* scheduler){
    ...
    int start=0;
    for (int i = 0; i <scheduler->copyQueueSize; i++) {
        currenttime+=scheduler->copyQueue[i].burstTime;
        printf("Process %d executed from %d to %d \n",(i+1),start,currenttime);
        scheduler->copyQueue[i].waitingTime=start;
        scheduler->copyQueue[i].completionTime=currenttime;
        start=currenttime;
        scheduler->copyQueue[i].turnaroundTime=start;
    }
    // Display completion time, turnaround time, and waiting time for each process
    printf("\nCompletion time\n");
    for (int i = 0; i < scheduler->copyQueueSize; i++) {
    ...
    printf("Average Turnaround Time = %.2f\n", (1.0)*sum/scheduler->copyQueueSize);
    turn[4]=(1.0)*sum/scheduler->copyQueueSize;

    printf("\nWaiting time\n");
    int sum2=0;
    for (int i = 0; i < scheduler->copyQueueSize; i++) {
        printf("Process %d waiting time %d\n", scheduler->copyQueue[i].processId, scheduler->copyQueue[i].waitingTime);
        sum2+=scheduler->copyQueue[i].waitingTime;
    }

    printf("Average Waiting Time = %.2f\n", (1.0)*sum2/scheduler->copyQueueSize);
    waiting[4]=(1.0)*sum2/scheduler->copyQueueSize;
